...
class QQWry(object):
    ...
    def query(self, ip):
        ...
        return (self.decode_str(c), self.decode_str(a))
    ...
    @staticmethod
    def decode_str(wry_str):
        try:
            return unicode(wry_str, 'gbk').encode('utf-8')
        except:
            # 当字符串解码失败，并且最一个字节值为'\x96',则去掉它，再解析
            if wry_str[-1] == '\x96':
                try:
                    return unicode(wry_str[:-1], 'gbk').encode('utf-8') + '?'
                except:
                    pass
